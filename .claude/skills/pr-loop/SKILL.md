...
3. **Verify before review**: `npm run build` (tsc is the completeness checklist for type changes), `npx vitest run` (ALL tests green — the current count lives in HANDOFF, not here), `npm run lint` (the only tolerated errors are the two pre-existing ones in App.tsx and useAuth.ts — anything else is yours). UI changes: drive the flow in Chrome with the test account (creds in `.secrets.local.json`; dev server usually lands on **5174**, trust the vite banner).
