...
- **Phase 3 data-contract columns are live and fully adopted** (applied 2026-07-03, UI shipped same day): `sets.rpe/notes/group_id/deleted_at` and `routine_exercises` all have UI. **Set deletion is SOFT everywhere** — `deleted_at` stamped via full-row outbox upsert in-workout (Undo clears it) and via `api/sets.deleteSet` in SessionDetail; every set-reading query carries `.is('deleted_at', null)` EXCEPT `fetchExportSets` (export keeps all rows and exposes the column). Any new set-reading query must add the filter. Still schema-only: `sessions.source` (default fills it), exercise metadata columns, case-insensitive name indexes.
